				...
				db.getLastReleaseVersion(res.env, (err, ver) => {
					...
					async.doUntil(deployRelease, checkNextDeploy, (err) => {
						...
						if(deployHelper.options.generate) {
							rarra.info('');
							rarra.info('Choose a backup to generate test data from');
							importData(() => {
								rarra.info('Test data generated');
								rarra.success();
							}, deployHelper.env);
						} else {
							rarra.success();
...
function doNotCleanGenerateOrTest() {
	...
	if(deployHelper.options.tests) {
		rarra.warn('Will not run tests');
	}

	deployHelper.options.clean = false;
	deployHelper.options.generate = false;
	deployHelper.options.tests = false;
}

function checkOptions(callback) {
	let env = rarra.getConfig().environments[deployHelper.env];
	let callDoNotClean = false;

	if(env.protected) {
		rarra.info(deployHelper.env + ' is protected environment.');
	...
	if(deployHelper.options.dryRun) {
		rarra.info('Doing a dry run. No changes are made to the database.');
