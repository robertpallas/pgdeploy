...
const copy = {
                ...
                this.getAndDropForeignKeys(dbHandler, (fkDropErr, foreignKeyQueries) => {
                    if(fkDropErr) {
                        this.dbDone(dbHandler, 'rollback;', done, () => {
                            ...
                            pgdeploy.error(`Failed to import data from backup ${backup} to ${envName}`);
                        });
                    } else {
                        async.eachSeries(tables, (table, asyncCb) => {
                                        ...
                                        async.each(foreignKeyQueries, (query, asyncCb) => {
                                            ...
                                            if(foreignKeyAddErr) {
                                                this.dbDone(dbHandler, 'rollback;', done, () => {
                                                    ...
                                                    pgdeploy.error(`Failed to add foreign keys after backup ${backup} to ${envName}`);
                                                });
                                            } else {
    ...
    checkColumns(tableDataFile, table, client, cb) {
        ...
        const fileRows = fileContents.split(/\r?\n/);
